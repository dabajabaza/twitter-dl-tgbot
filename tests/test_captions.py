"""The delivery caption: post text, an author footer, and Telegram's limit."""
...
def caption(description: str) -> str:
    return build_caption(
        description, TWEET, provider=PROVIDER, uploader="cats", uploader_url=PROFILE
    )
...
class TestShape:
    def test_a_post_without_text_is_just_the_footer(self) -> None:
...
class TestEscaping:
    def test_html_in_the_post_text_cannot_become_markup(self) -> None:
        built = caption('<b>&"bold"</b>')
        assert built.startswith("&lt;b&gt;&amp;&quot;bold&quot;&lt;/b&gt;")

    def test_html_in_the_handle_cannot_become_markup(self) -> None:
        built = build_caption("", TWEET, provider=PROVIDER, uploader="a<b>&c", uploader_url=PROFILE)
    ...
    def test_an_ampersand_in_either_url_is_escaped_inside_href(self) -> None:
        built = build_caption(
            ...
            provider=PROVIDER,
            uploader="cats",
            uploader_url="https://x.com/u?a&b",
        )
